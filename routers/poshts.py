...
from schemas import PoshtCreate, PoshtRead, PoshtUpdate

router = APIRouter(prefix="/poshts", tags=["poshts"])


@router.get("/", response_model=List[PoshtRead], tags=["poshts"])
async def get_poshts(db: AsyncSession = Depends(get_db)) -> List[PoshtRead]:
    return await get_poshts_from_db(db)


@router.get("/{posht_id}", response_model=PoshtRead, tags=["poshts"])
async def get_posht(posht_id: int, db: AsyncSession = Depends(get_db)) -> PoshtRead:
...
async def update_posht(
    posht_id: int, posht: PoshtUpdate, db: AsyncSession = Depends(get_db)
) -> PoshtRead:
...
@router.delete(
    "/{posht_id}", response_model=PoshtRead, dependencies=[Depends(require_admin)]
)
async def remove_posht(posht_id: int, db: AsyncSession = Depends(get_db)) -> PoshtRead:
